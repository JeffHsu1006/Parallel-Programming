...
void cal_man_set (int* image, int local_start, int sz);
...
int main(int argc, char** argv) {
    ...
    /* allocate memory for global image */
    if(rank==0) global_image = malloc(width * height * sizeof(int));
        
    assert(image);
    double start = MPI_Wtime();
    cal_man_set(image, local_start, size);
    MPI_Barrier(MPI_COMM_WORLD);
    computing_time += MPI_Wtime() - start;
    printf("rank %d computing time: %f\n",rank, computing_time);

    MPI_Reduce(image, global_image, width*height, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
...
void cal_man_set(int* image, int local_start, int sz){
    /* mandelbrot set */
    int repeats,i,j;
    double x,y,x0,y0,temp,length_squared;
    
    #pragma omp parallel for schedule(dynamic)
    for (j = local_start; j < height; j+=sz) {
